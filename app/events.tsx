...
const TYPE_EMOJIS: Record<string, string> = {
  'Fly-In': '✈️', 'Airshow': '🛩️', 'Pancake Breakfast': '🥞',
  'Poker Run': '🃏', 'EAA Event': '🔧', 'AOPA Event': '🏆', 'Other': '📍'
};
...
export default function EventsScreen() {
  ...
  const [form, setForm] = useState({
    icao: '', airport_name: '', city: '', state: '',
    title: '', type: 'Fly-In', description: '', date: '', time: ''
  });
  ...
  async function toggleInterested(eventId: string) {
    ...
    if (isIn) {
      ...
      await supabase.from('events').update({ interested_count: (events.find(e=>e.id===eventId)?.interested_count||1)-1 }).eq('id', eventId);
      ...
      await supabase.from('events').update({ interested_count: (events.find(e=>e.id===eventId)?.interested_count||0)+1 }).eq('id', eventId);
  ...
  async function addToCalendar(event: any) {
    ...
    try {
      ...
      await Calendar.createEventAsync(defaultCal.id, {
        ...
        startDate: eventDate,
        endDate: endDate,
        location: `${event.airport_name} (${event.icao}), ${event.city}, ${event.state}`,
  ...
  async function submitEvent() {
    if (!form.icao || !form.title || !form.date) {
      Alert.alert('Missing info', 'Please fill in ICAO, event title, and date.');
      return;
    }
    setSubmitting(true);
  ...
  <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterRow} contentContainerStyle={{ paddingHorizontal: 20, gap: 8 }}>
        {EVENT_TYPES.map(t => (
          <TouchableOpacity
            key={t}
            style={[styles.chip, activeFilter === t && styles.chipActive]}
            onPress={() => setActiveFilter(t)}
          >
            <Text style={[styles.chipText, activeFilter === t && styles.chipTextActive]}>
              {t !== 'All' ? (TYPE_EMOJIS[t] + ' ') : ''}{t}
            </Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
              ...
              <View key={event.id} style={styles.card}>
                ...
                <View style={styles.cardTop}>
                  <View style={styles.typeBadge}>
                    <Text style={styles.typeBadgeText}>{TYPE_EMOJIS[event.type] || '📍'} {event.type}</Text>
                  </View>
                ...
                <Text style={styles.cardAirport}>
                  {event.icao} · {event.airport_name}
                </Text>
                <Text style={styles.cardMeta}>
                  📅 {formatDate(event.date)}{event.time ? `  ·  🕐 ${event.time}` : ''}
                </Text>
                {dist !== null && (
                  <Text style={styles.cardMeta}>📍 {event.city}, {event.state}  ·  {dist} nm away</Text>
                )}
                ...
                <View style={styles.cardActions}>
                  ...
                  >
                    <Text style={[styles.actionBtnText, isInterested && styles.actionBtnTextActive]}>
                      {isInterested ? '⭐ Interested' : '☆ Interested'}
                    </Text>
                    {event.interested_count > 0 && (
                      <Text style={[styles.actionCount, isInterested && { color: '#0D1421' }]}>
                ...
                {/* View Airport */}
                <TouchableOpacity
                  style={styles.viewAirportBtn}
                  onPress={() => router.push({ pathname: '/airport', params: { icao: event.icao, name: event.airport_name, city: event.city, state: event.state, lat: event.lat, lng: event.lng } })}
                >
      ...
      <Modal visible={showSubmit} animationType="slide" presentationStyle="pageSheet">
        <View style={styles.modal}>
          ...
          <ScrollView contentContainerStyle={styles.modalBody}>
            ...
            <TextInput style={styles.input} placeholder="e.g. KSUS" placeholderTextColor="#4A5B73"
              value={form.icao} onChangeText={v => setForm(f => ({ ...f, icao: v }))} autoCapitalize="characters" />

            <Text style={styles.fieldLabel}>Airport Name</Text>
            <TextInput style={styles.input} placeholder="e.g. Spirit of St. Louis" placeholderTextColor="#4A5B73"
              value={form.airport_name} onChangeText={v => setForm(f => ({ ...f, airport_name: v }))} />

            <View style={{ flexDirection: 'row', gap: 12 }}>
              <View style={{ flex: 1 }}>
                <Text style={styles.fieldLabel}>City</Text>
                <TextInput style={styles.input} placeholder="City" placeholderTextColor="#4A5B73"
                  value={form.city} onChangeText={v => setForm(f => ({ ...f, city: v }))} />
              </View>
              <View style={{ width: 80 }}>
                <Text style={styles.fieldLabel}>State</Text>
                <TextInput style={styles.input} placeholder="MO" placeholderTextColor="#4A5B73"
                  value={form.state} onChangeText={v => setForm(f => ({ ...f, state: v }))} autoCapitalize="characters" />
              </View>
            </View>

            <Text style={styles.fieldLabel}>Event Title *</Text>
            <TextInput style={styles.input} placeholder="e.g. EAA Chapter 54 Pancake Breakfast" placeholderTextColor="#4A5B73"
              value={form.title} onChangeText={v => setForm(f => ({ ...f, title: v }))} />

            <View style={{ height: 44, marginBottom: 12 }}>
  <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20, gap: 8, alignItems: 'center', flexGrow: 1 }}>
    {EVENT_TYPES.map(t => (
      <TouchableOpacity key={t} style={[styles.chip, activeFilter === t && styles.chipActive]} onPress={() => setActiveFilter(t)}>
        <Text style={[styles.chipText, activeFilter === t && styles.chipTextActive]}>{t}</Text>
            ...
            <TextInput style={styles.input} placeholder="2026-05-10" placeholderTextColor="#4A5B73"
              value={form.date} onChangeText={v => setForm(f => ({ ...f, date: v }))} />

            <Text style={styles.fieldLabel}>Time</Text>
            <TextInput style={styles.input} placeholder="e.g. 8:00 AM - 11:00 AM" placeholderTextColor="#4A5B73"
              value={form.time} onChangeText={v => setForm(f => ({ ...f, time: v }))} />

            <Text style={styles.fieldLabel}>Description</Text>
            <TextInput style={[styles.input, { height: 90, textAlignVertical: 'top' }]}
              placeholder="Tell pilots what to expect..." placeholderTextColor="#4A5B73"
            ...
            <TouchableOpacity style={styles.submitBtn} onPress={submitEvent} disabled={submitting}>
              {submitting
                ...
                : <Text style={styles.submitBtnText}>Submit Event ✈️</Text>}
            </TouchableOpacity>
            <View style={{ height: 40 }} />
          </ScrollView>
        </View>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', paddingHorizontal: 20, paddingTop: 16, paddingBottom: 20 },
...
filterRow: { marginBottom: 12, marginTop: 4, maxHeight: 44 },
 chip: { backgroundColor: '#0D1421', borderRadius: 20, paddingHorizontal: 14, paddingVertical: 8, borderWidth: 1, borderColor: '#1E2D45', alignSelf: 'flex-start' },
